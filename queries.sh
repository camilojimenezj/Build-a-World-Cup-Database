...
echo "$($PSQL "(SELECT name FROM teams JOIN games on games.winner_id = teams.team_id WHERE round='Eighth-Final' AND year=2014) UNION 
      (SELECT name FROM teams JOIN games on games.opponent_id = teams.team_id WHERE round='Eighth-Final' AND year=2014) ORDER BY name")"
